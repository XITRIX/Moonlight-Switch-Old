...
bool WakeOnLanManager::wake_up_host(const Host &host) {
    ...
    if (getaddrinfo(host.address.c_str(), NULL, &hints, &res) != 0 || res == NULL) {
        ...
        lg::Logger::error("WakeOnLanManager", "Failed to resolve WOL address");
        return result;
    }
    ...
    for (curr = res; curr != NULL; curr = curr->ai_next) {
        ...
        if (wol_socket < 0) {
            lg::Logger::error("WakeOnLanManager", "Failed to create WOL socket");
            continue;
        }
        ...
        for (int j = 0; j < numPorts; j++) {
            ...
            result = result || err > 0;
            lg::Logger::info("WakeOnLanManager", "Sending WOL packet returned: %ld", err);
        }
