...
int AudrenAudioRenderer::init(int audio_configuration, const POPUS_MULTISTREAM_CONFIGURATION opus_config, void *context, int ar_flags) {
    ...
    lg::Logger::info("Audren", "Init with channels: %i, sample rate: %i", m_channel_count, m_sample_rate);
    ...
    if (!mempool_ptr) {
        lg::Logger::error("Audren", "mempool alloc failed");
        return -1;
    }
    
    Result rc = audrenInitialize(&m_ar_config);
    if (R_FAILED(rc)) {
        lg::Logger::error("Audren", "audrenInitialize: %x", rc);
        return -1;
    }
    
    rc = audrvCreate(&m_driver, &m_ar_config, m_channel_count);
    if (R_FAILED(rc)) {
        lg::Logger::error("Audren", "audrvCreate: %x", rc);
        return -1;
    }
    ...
    if (R_FAILED(rc)) {
        lg::Logger::error("Audren", "audrenStartAudioRenderer: %x", rc);
    }
    ...
    lg::Logger::info("Audren", "Init done!");
    
    return DR_OK;
}

void AudrenAudioRenderer::cleanup() {
    lg::Logger::info("Audren", "Cleanup...");
    ...
    lg::Logger::info("Audren", "Cleanup done!");
}
...
void AudrenAudioRenderer::decode_and_play_sample(char *data, int length) {
    if (m_decoder && m_decoded_buffer) {
        ...
        lg::Logger::fatal("Audren", "Invalid call of decode_and_play_sample");
    }
}
...
void AudrenAudioRenderer::write_audio(const void *buf, size_t size) {
    if (!m_inited_driver) {
        lg::Logger::fatal("Audren", "Call write_audio without init driver!");
        return;
    }
