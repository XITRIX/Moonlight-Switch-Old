...
int FFmpegVideoDecoder::setup(int video_format, int width, int height, int redraw_rate, void *context, int dr_flags) {
    ...
    if (m_decoder == NULL) {
        lg::Logger::error("FFmpeg", "Couldn't find decoder");
        return -1;
    }
    
    m_decoder_context = avcodec_alloc_context3(m_decoder);
    if (m_decoder_context == NULL) {
        lg::Logger::error("FFmpeg", "Couldn't allocate context");
        return -1;
    }
    ...
    if (err < 0) {
        lg::Logger::error("FFmpeg", "Couldn't open codec");
        return err;
    }
    ...
    if (m_frames == NULL) {
        lg::Logger::error("FFmpeg", "Couldn't allocate frames");
        return -1;
    }
    ...
    for (int i = 0; i < m_frames_count; i++) {
        ...
        if (m_frames[i] == NULL) {
            lg::Logger::error("FFmpeg", "Couldn't allocate frame");
            return -1;
        }
    }
    ...
    if (m_ffmpeg_buffer == NULL) {
        lg::Logger::error("FFmpeg", "Not enough memory");
        cleanup();
        return -1;
    }
...
int FFmpegVideoDecoder::submit_decode_unit(PDECODE_UNIT decode_unit) {
    if (decode_unit->fullLength < DECODER_BUFFER_SIZE) {
        ...
        if (length > DECODER_BUFFER_SIZE) {
            lg::Logger::error("FFmpeg", "Big buffer to decode...");
        }
...
int FFmpegVideoDecoder::decode(char* indata, int inlen) {
    ...
    if (err < 0) {
        ...
        lg::Logger::error("FFmpeg", "Decode failed - %s", error);
    }
...
AVFrame* FFmpegVideoDecoder::get_frame(bool native_frame) {
    ...
    if (err == 0) {
        ...
        char error[512];
        av_strerror(err, error, sizeof(error));
        lg::Logger::error("FFmpeg", "Receive failed - %d/%s", err, error);
    }
    return NULL;
}
