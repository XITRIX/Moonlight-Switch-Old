...
int gs_pair(PSERVER_DATA server, char* pin) {
    ...
    lg::Logger::info("Client", "Pairing with generation %d server", server->serverMajorVersion);
    lg::Logger::info("Client", "Start pairing stage #1");
    
    Data salt = Data::random_bytes(16);
    Data salted_pin = salt.append(Data(pin, strlen(pin)));
    lg::Logger::info("Client", "PIN: %s, salt %s", pin, salt.hex().bytes());
    ...
    lg::Logger::info("Client", "Start pairing stage #2");
    ...
    lg::Logger::info("Client", "Start pairing stage #3");
    ...
    lg::Logger::info("Client", "Start pairing stage #4");
    ...
    lg::Logger::info("Client", "Start pairing stage #5");
...
int gs_init(PSERVER_DATA server, char *address, const char *keyDirectory, bool unsupported) {
    ...
    if (!CryptoManager::load_cert_key_pair()) {
        ...
        if (!CryptoManager::generate_new_cert_key_pair()) {
            lg::Logger::info("Client", "Failed to generate certs...");
            return GS_FAILED;
        }
    }
